...
import { csv as moduscsv, html as modushtml, file as modusfile } from '@modusjs/convert';
...
program
  ...
  .action(async (filenames: string[], opts) => {
    ...
    const results = await modusfile.fromFile(filenames.map(filename => ({ ...base, filename })));
    ...
    for (const { original_filename, output_filename, modus } of results) {
      try {
        ...
        const towrite = opts.compact ? JSON.stringify(modus) : JSON.stringify(modus,null,'  ');
        await fs.writeFile(output_filename, towrite);
        info('Successfully converted', original_filename, 'into ', output_filename);
      } catch (e: any) {
...
program
  ...
  .action(async (filenames: string[], opts) => {
    ...
    info('Parsed', cyan(results.length), ' Modus JSON files for inclusion in CSV output');
    ...
    info('Successfully wrote CSV output to', opts.output, 'from', cyan(results.length), 'Modus results');
  });
...
program
  ...
  .action(async (filenames: string[], opts) => {
    ...
    const results = await modusfile.fromFile(filenames.map(filename => ({ ...base, filename })));
    // flatten all results from all files into one array of ModusResults:
    info('Parsed', cyan(results.length), ' Modus JSON files for HTML conversion');
    ...
    for (const { original_filename, output_filename, modus } of results) {
      try {
        ...
        info('Successfully converted', original_filename, 'into', html_filename);
      } catch (e: any) {
        if (e.errors  && e.input && Array.isArray(e.errors)) { // AJV error

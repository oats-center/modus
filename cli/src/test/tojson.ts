...
export default async function run() {
  ...
  test('tojson result test-work/hand-modus.json matched expected-hand-modus.json');
  const result = JSON.parse((await fs.readFile('./test-work/hand-modus.json')).toString());
  const expected = JSON.parse((await fs.readFile('./test-work/expected-hand-modus.json')).toString());

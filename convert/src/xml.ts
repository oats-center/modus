...
// }
type KeyedSetOverride = {
  // <Reports><Report ReportID="1">...</Report></Reports> => { 1: ... }
  type: 'keyedSet';
  id_attrib: string;
};
...
type AccumulateArrayOverride = {
  // <SoilSample>...</SoilSample><SoilSample></SoilSample> => [accumulatorKeyname]: [ ... ],
  type: 'accumulateArray';
  finalKeynameForArrayInParent: string; // SoilSamples
};
type ArrayOverride = {
  // <NutrientResults><NutrientResult>...</NutrientResult></NutrientResults> => { NutrientResults: [ ... ] }
  type: 'array';
};
type EmptyTagsBecomeTrueOverride = { type: 'emptyTagsBecomeTrue' };
type Overrides = {
  [key_to_override: string]:
    | KeyedSetOverride
    | AccumulateArrayOverride
    | ArrayOverride
    | EmptyTagsBecomeTrueOverride;
};
type ParseOptions = {
  overrides?: Overrides;
...
export function parse(xmlstring: string): any {
    ...
    for (const child of xml.children) {
      ...
      if (!id) {
        warn(
          ...
          'at path',
          path,
          'does not contain required ID attribute ',
          id_attrib,
          ', ignoring'
        );
  ...
  function parseStringOrNumberOrBoolean({
    str,
    tagname,
    path,
    opts,
    ...
    const parse_as_number =
      !!opts.parseAsNumbers[tagname] &&
...
function exactlyOneTag(xml: Cheerio<AnyNode>): Element | null {
  ...
  if (all.length > 1) {
    warn(
      ...
      `Tag ${
        all[0]!.tagName
      } can only exist once at this level, but multiple tags found.  Only using first one.`
    );
  }
  return all[0]!;
}